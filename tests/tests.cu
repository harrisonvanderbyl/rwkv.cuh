...
//     Tensor bias_bf16 = {{4096}, TENSORTYPE::kBFLOAT_16};
//     for (int i = 0; i < 4096; i++){
//         ((bfloat16*)bias_bf16.data)[i] = bfloat16(((float*)bias.data)[i]);
//     }
//     Tensor weight_bf16 = {{4096}, TENSORTYPE::kBFLOAT_16};
//     for (int i = 0; i < 4096; i++){
//         ((bfloat16*)weight_bf16.data)[i] = bfloat16(((float*)weight.data)[i]);
//     }
//     Tensor input_bf16 = {{2,4096}, TENSORTYPE::kBFLOAT_16};
//     for (int i = 0; i < 2*4096; i++){
...
//     }
//    // bfloat16 test
//     Tensor rtest_bf16 = {{4096}, TENSORTYPE::kBFLOAT_16};
...
//     // bfloat16 test
//     Tensor ltest_bf16 = {{4096}, TENSORTYPE::kBFLOAT_16};

//     for (int i = 0; i < 4096; i++){
//         ((bfloat16*)ltest_bf16.data)[i] = bfloat16(((float*)ltest.data)[i]);
//     }
//     Tensor ltest2_bf16 = {{4096}, TENSORTYPE::kBFLOAT_16};
//     for (int i = 0; i < 4096; i++){
...
//     // bfloat16 test
//     Tensor wtest_bf16 = {{4096}, TENSORTYPE::kBFLOAT_16};
//     Tensor wtest2_bf16 = {{4096}, TENSORTYPE::kBFLOAT_16};
//     for (int i = 0; i < 4096; i++){
...
//     std::cout << wkvtest_bf16_cuda << std::endl;
    ...
    Tensor matmultest = {{4096,4096}};
    Tensor matmultest_bf16 = {{4096,4096}, TENSORTYPE::kBFLOAT_16};
    Tensor matmultest_uint8 = {{4096,4096}, TENSORTYPE::kUINT_8};
    Tensor matmultest_uint8_range = {{4096}, TENSORTYPE::kFLOAT_32};
    Tensor matmultest_uint8_offset = {{4096}, TENSORTYPE::kFLOAT_32};
    ...
    for (int i = 0; i < 4096; i++){
        ...
        for (int j = 0; j < 4096; j++){
            ((float*)matmultest.data)[i*4096+j] = float(rand())/float(RAND_MAX);
            ((bfloat16*)matmultest_bf16.data)[i*4096+j] = bfloat16(((float*)matmultest.data)[i*4096+j]);
            if (((float*)matmultest.data)[i*4096+j] < min){
                min = ((float*)matmultest.data)[i*4096+j];
            }
            if (((float*)matmultest.data)[i*4096+j] > max){
                max = ((float*)matmultest.data)[i*4096+j];
            }
        }

        ((float*)matmultest_uint8_range.data)[i] = (max - min)/255.f;
        ((float*)matmultest_uint8_offset.data)[i] = min;

        for (int j = 0; j < 4096; j++){
            ((uint8_t*)matmultest_uint8.data)[i*4096+j] = uint8_t((((float*)matmultest.data)[i*4096+j] - min)/((max - min)/255.f));
        }
        
    }

    Tensor matmultest2 = {{1,18,4096}};
    Tensor matmultest2_bf16 = {{1,18,4096}, TENSORTYPE::kBFLOAT_16};
    for (int i = 0; i < 18*4096; i++){
        ((float*)matmultest2.data)[i] = 0.001* float(rand())/float(RAND_MAX);
    ...
    // std::cout << "Matmultestprematmul3" << std::endl;
    auto outputmm_uint8 = matmultest_uint8.matmul(matmultest_uint8_range,matmultest_uint8_offset,matmultest2);
    ...
    std::cout << outputmm_uint8 << std::endl;
    
    // matmultest cuda
    std::cout << "matmultest cuda" << std::endl;
    Tensor matmultest_cuda = matmultest.cuda();
    Tensor matmultest2_cuda = matmultest2.cuda();
    Tensor matmultest_uint8_cuda = matmultest_uint8.cuda();
    Tensor matmultest_uint8_range_cuda = matmultest_uint8_range.cuda();
    Tensor matmultest_uint8_offset_cuda = matmultest_uint8_offset.cuda();
    ...
    auto outputmm_uint8_cuda = matmultest_uint8_cuda.matmul(matmultest_uint8_range_cuda,matmultest_uint8_offset_cuda,matmultest2_cuda);
    std::cout << outputmm_cuda << std::endl;
    std::cout << outputmm_bf16_cuda << std::endl;
    std::cout << outputmm_uint8_cuda << std::endl;
    ...
    // Tensor a = {{65536,4096},TENSORTYPE::kBFLOAT_16};
    // for(size_t i = 0; i < 65536*4096; i+=32){
    //     if(i%(65536*4) == 0){
    //         std::cout << "\r" << "Progress: [";
    //         for (int j = 0; j < 20; j++){

    //             if (float(j*5) < 100.0f * float(i/4096)/float(65536)){
    ...
    //             }
    //         }
    //         std::cout << "] " << float(i)/float(65336*4096)*100.0f << "%";

    //     }
    //     __m512i a1 = _mm512_set1_epi64(rand());
    //     auto a2 = _mm512_cvtepu32_ps(a1);
    //     a2 = _mm512_mul_ps(a2,_mm512_set1_ps(0.00000000001));
    //     auto a3 = _mm512_cvtne2ps_pbh(a2,a2);

    //     _mm512_storeu_si512((__m512i*)&((bfloat16*)a.data)[i],*(__m512i*)&a3);

    // }

    // Tensor b = {{1,18,4096},TENSORTYPE::kBFLOAT_16};
    // for(size_t i = 0; i < 18*4096; i++){
